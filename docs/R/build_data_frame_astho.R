...
df$infl_adj <- adjust_for_inflation(df$value,
                                    df$year,
                                    country,
                                    to_date = 2018)
...
df.2 <-
        ...
        summarize(tot_infl_adj = sum(infl_adj, na.rm = T),
                  tot_pop = sum(pop)
                  ) %>%
        mutate(spend_per_cap = divide_by(tot_infl_adj, tot_pop) %>% round(2))
...
ggplot(df.2, aes(year, spend_per_cap, total, group = variable, color = variable)) +
geom_line() +
geom_point() +
